...
def managed_component_logs(cmd, client, resource_group, service,
                           ...
                           follow=None, lines=50, since=None, limit=2048):
    ...
    if not name and instance:
        ...
        threads = _get_log_threads(url_dict, auth, exceptions)
...
def _get_log_threads(url_dict, auth, exceptions):
    threads = []
    need_prefix = True if len(url_dict) > 1 else False
...
def _sequential_start_threads(threads: [Thread]):
    for idx, t in enumerate(threads):
        ...
        if idx != len(threads) - 1:
            # Print empty lines as separator
            print("\n\n")
...
def _get_log_threads_without_component(cmd, client, resource_group, service, instance_name, auth, exceptions, queryOptions: QueryOptions):
    ...
    return [Thread(target=log_stream_from_url, args=(url, auth, None, exceptions, DefaultWriter()))]
